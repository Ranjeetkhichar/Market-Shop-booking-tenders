...
if($_SERVER["REQUEST_METHOD"] == "POST")
{
  ...
  $month = $_POST["month"];
  $ebillstatus = $_POST["e-billstatus"];
  $rentstatus = $_POST["rentstatus"];

  if(empty($contractid) || empty($month) || empty($ebillstatus) || empty($rentstatus))
  {
    $emptyerror = true;
  }
  else
  {
    ...
    if($ebillstatus != 'NA')
    {
        $query = "UPDATE payment SET electricitybillstatus = '$ebillstatus' WHERE contractid = $contractid AND payment.month = '$month'";
        mysqli_query($db,$query) or die(mysqli_error($db));
    }

    if($rentstatus != 'NA')
    {
        $query = "UPDATE payment SET rentstatus = '$rentstatus' WHERE contractid = $contractid AND payment.month = '$month'";
        mysqli_query($db,$query) or die(mysqli_error($db));
    }
...
<nav class="navbar navbar-expand-lg navbar-dark bg-dark">
  ...
  <form action="updatebills.php" method="POST">
    <?php
      ...
      <div class="col">
        <label for="e-billstatus">Electricity Bill</label>
        <select id="e-billstatus" name="e-billstatus" class="form-control">
      ...
      <div class="col">
        <label for="rentstatus">Rent Paid</label>
        <select id="rentstatus" name="rentstatus" class="form-control">
            <option selected value = "NA"> -- NA -- </option>
            <option value="PENDING">PENDING</option>
            <option value="DONE">DONE</option>
        </select>
